...
export default function Alltodos ({data}){
    ...
    <div className="container px-5 py-12 mx-auto flex flex-wrap">
      <div className="flex flex-wrap w-full">
        <div className="p-2 lg:w-1/2 md:w-full">
          <h2 className='text-xl font-semibold mb-2 text-center text-amber-300'>Pending ToDos - {data.length}</h2>
          {
            data.map((todos,i) => (
...
Alltodos.Layout = Dlayout;

export async function getServerSideProps() {
  const res = await fetch(`http://localhost:3000/api/pendingTask`)
